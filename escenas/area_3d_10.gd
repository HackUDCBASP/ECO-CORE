...
@onready var item_scene = preload("res://modelos/trash/manzana.glb")
...
func _unhandled_input(event):
	if can_interact and not collected and event.is_action_pressed(interact_action):
		...
		if player:
			...
			item_instance.add_to_group("manzana")
			item_instance.position = Vector3(0, 1.5, 0)
		# Eliminar el objeto del suelo por completo
		queue_free()
		print("Has recogido una manzana")
